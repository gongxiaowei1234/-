  ...
  <div class="detail">
    ...
    <section class="con">
      ...
      <div class="conPoin">
        ...
        <span v-show="detailList.categoryView.category2Name">{{
          detailList.categoryView.category2Name
        }}</span>
        <span v-show="detailList.categoryView.category3Name">{{
          detailList.categoryView.category3Name
        }}</span>
      </div>
      <!-- 主要内容区域 -->
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl
                v-for="(spuSaleAttr, index) in detailList.spuSaleAttrList"
                :key="spuSaleAttr.id"
              >
            ...
            <div class="cartWrap">
              <div class="controls">
                <!-- 收集表单数据有两种,一种v-model,一种ref,
                v-model获取的数据是number，而ref获取的则是一个字符串 -->
                <input
                  autocomplete="off"
                  class="itxt"
                  v-model="skuNum"
...
import { reqGetDetailInfo, reqGetShopCartNumInfo } from "@/api";
// 引入封装的游客身份模块uuid
...
export default {
  ...
  created() {
    this.GetDetailInfo();
    // console.log(localStorage.getItem("UUIDTOKEN"));
  ...
  methods: {
    ...
    GetDetailInfo() {
      ...
      // console.log(skuId);
      reqGetDetailInfo(skuId).then((res) => {
        // console.log(res);
        const { categoryView, price, skuInfo, spuSaleAttrList, valuesSkuJson } =
          res.data;
